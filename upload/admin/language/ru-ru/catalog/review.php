<?php
// Heading
$_['heading_title']     = 'Отзывы';

// Text
$_['text_success']      = 'Настройки успешно изменены!';
$_['text_list']         = 'Отзывы';
$_['text_add']          = 'Добавить';
$_['text_edit']         = 'Редактирование';
$_['text_filter']       = 'Фильтр';
$_['text_files']               = 'файлов';
$_['text_click_or_drag_n_drop']= 'Нажмите на кнопку или перетащите фото в эту область';
$_['text_max_images']          = 'До 10 изображений в формате PNG, JPEG';
$_['text_u_cant_load_more']    = 'Вы не можете загружать больше';
$_['text_images']              = 'изображений';
$_['text_was_selected_one_file'] = 'Был выбран 1 файл';
$_['text_was_selected_files']  = 'файлов были выбраны';

// Column
$_['column_product']    = 'Товар';
$_['column_author']     = 'Автор';
$_['column_rating']     = 'Рейтинг';
$_['column_status']     = 'Статус';
$_['column_date_added'] = 'Дата';
$_['column_action']     = 'Действие';

// Entry
$_['entry_product']     = 'Товар';
$_['entry_author']      = 'Автор';
$_['entry_rating']      = 'Рейтинг';
$_['entry_images']      = 'Изображения отзыва';
$_['entry_hidden_customer_info'] = 'Скрыть данные пользователя';
$_['entry_status']      = 'Статус';
$_['entry_text']        = 'Текст';
$_['entry_date_added']  = 'Дата';

// Buttons
$_['button_delete_image'] = 'Удалить изображение';
$_['button_delete_all']   = 'Удалить все';

// Confirm
$_['confirm_delete_image'] = 'Вы уверены, что хотите удалить это изображение?';

// Help
$_['help_product']      = '(Автозаполнение)';

// Error
$_['error_permission']  = 'У Вас нет прав для изменения отзывов!';
$_['error_product']     = 'Требуется выбрать товар!';
$_['error_author']      = 'Имя автора должно содержать от 3 до 64 символов!';
$_['error_text']        = 'Текст отзыва должен содержать хотя бы 1 символ!';
$_['error_rating']      = 'Требуется установить рейтинг!';
$_['error_only_image']  = 'Загружаются только изображения!';
$_['error_more_image']  = 'Загрузите меньше 10 изображений!';

