...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
            ...
            $data['write_review_btn'] = $this->url->link('product/product/writeReview', 'product_id=' . $this->request->get['product_id'] . $url);
	...
	public function review() {
		...
		foreach ($results as $result) {
            ...
            if ($review_addiction_info['hidden_customer_info'] == "0") {
                $author_name = $result['author'];
                $first_letter_author = isset($result['author']) ? utf8_strtoupper(utf8_substr($result['author'],0,1)) : '';
            } else {
                $author_name = $this->language->get('text_anonymous');
	...
	public function writeReview() {
        ...
        if ($product_info) {
            ...
            $data['weight_unit'] = $product_info['weight_unit'];
            ...
            $this->response->setOutput($this->load->view('product/review_form', $data));
        } else {
            $url = '';
	...
	public function write() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!isset($json['error'])) {
				...
				$json['review_id'] = $this->model_catalog_review->addReview($this->request->get['product_id'], $this->request->post);

				if (isset($this->request->post['hidden_customer_info'])) {
                    $this->model_catalog_review->addReviewAddictionInfo($json['review_id'], 1);
                } else {
                    $this->model_catalog_review->addReviewAddictionInfo($json['review_id'], 0);
                }
